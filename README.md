...
- Convert the static html list into dynamic one by:
  - creating `ItemListCtrl` controller for the application.
...
- Introduce the [$route] service which allows binding URLs for deep-linking with
  views:
  - Create `itemcatCtrl` which governs the entire app and contains $route configuration.
...
- Create phone list route:
  - Map `/phones` route to `ItemListCtrl` and `partials/phones-list.html`.
  - Preserve existing `ItemListCtrl` controller.
  - Move existing html from `index.html` to `partials/phone-list.html`.
- Create phone details route:
  - Map `/phones/<phone-id>` route to `ItemDetailCtrl` and `partials/phones-detail.html`.
...
- Implement `ItemDetailCtrl` controller to fetch the details for a specific phone from a JSON file
  using `$http` service.
...
- Define `mainImageUrl` model variable in the `ItemDetailCtrl` and set its default value.
...
## Development with angular-itemcat
